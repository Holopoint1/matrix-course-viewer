...
function realName(folderCode, name) {
  ...
  /* STRICT matching: resolve ONLY to the exact name or a trivially-equivalent
     spelling (letter-case, en/em-dash vs hyphen, .htm↔.html, .jpg↔.jpeg,
     .tif↔.tiff). There is no base-name / cross-extension fallback — a screen that
     asks for "X.xlsx" can only ever resolve to a spreadsheet of that name, never a
     same-named .htm/.docx/etc. Anything not present is reported MISSING so the Drive
     filename (e.g. a CO002 vs CO0002 typo) gets corrected rather than silently
     substituted with the wrong file. */
